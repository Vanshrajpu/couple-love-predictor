...
.metric-lbl{font-size:10px; font-weight:800; color:#64748b; text-transform:uppercase; letter-spacing:0.8px;}

.stSlider label{display:none!important;}
div[data-baseweb="slider"] > div{padding:0 10px!important;}
...
.factor-label{font-size:13px; font-weight:700; color:#0f172a; display:flex; gap:6px;}
...
<div style="background:white; border:1px solid #e8edf5; border-radius:18px; padding:16px 22px; display:flex; justify-content:space-between; align-items:center; box-shadow:0 8px 32px rgba(0,0,0,0.06); margin-bottom:16px;">
  <div style="display:flex; gap:14px; align-items:center;">
    ...
    <div><div style="font-size:18px; font-weight:800; color:#0f172a; letter-spacing:-0.4px;">BondIQ — Couple Compatibility Engine</div><div style="font-size:11px; color:#64748b; font-weight:500;">Explainable AI • 91.2% Accuracy • Production Ready • Vansh Rajput</div></div>
  </div>
...
with c1: st.markdown('<div class="metric" style="--accent:#3b82f6;"><div class="metric-lbl">🎯 Accuracy</div><div class="metric-val">91.2%</div><div style="font-size:11px; color:#16a34a; font-weight:600;">↑ High</div></div>', unsafe_allow_html=True)
...
with left:
    ...
    st.markdown('<div style="font-weight:800; font-size:16px; color:#0f172a; margin-bottom:4px;">🧩 Relationship Factors</div><div style="font-size:12px; color:#64748b; margin-bottom:22px;">Adjust — BondIQ updates instantly from your couple_love_model.pkl</div>', unsafe_allow_html=True)
    L,R = st.columns(2, gap="large")
    with L:
        comm = st.slider("comm", 0.0, 10.0, 8.2, key="c1")
        st.markdown(f'<div class="factor-row" style="margin:-10px 0 18px 0;"><div class="factor-label">💬 Communication</div><div class="factor-val">{comm:.1f} / 10</div></div>', unsafe_allow_html=True)
        und = st.slider("und", 0.0, 10.0, 8.1, key="c2")
        st.markdown(f'<div class="factor-row" style="margin:-10px 0 18px 0;"><div class="factor-label">🧠 Understanding</div><div class="factor-val">{und:.1f} / 10</div></div>', unsafe_allow_html=True)
        supp = st.slider("supp", 0.0, 10.0, 8.3, key="c3")
        st.markdown(f'<div class="factor-row" style="margin:-10px 0 18px 0;"><div class="factor-label">🤝 Support</div><div class="factor-val">{supp:.1f} / 10</div></div>', unsafe_allow_html=True)
        gift = st.slider("gift", 0.0, 15.0, 6.0, key="c4")
        st.markdown(f'<div class="factor-row" style="margin:-10px 0 0 0;"><div class="factor-label">🎁 Gifts / Month</div><div class="factor-val">{gift:.0f}</div></div>', unsafe_allow_html=True)
    with R:
        trust = st.slider("trust", 0.0, 10.0, 8.5, key="c5")
        st.markdown(f'<div class="factor-row" style="margin:-10px 0 18px 0;"><div class="factor-label">🛡️ Trust</div><div class="factor-val">{trust:.1f} / 10</div></div>', unsafe_allow_html=True)
        time = st.slider("time", 0.0, 168.0, 35.0, key="c6")
        st.markdown(f'<div class="factor-row" style="margin:-10px 0 18px 0;"><div class="factor-label">⏳ Time Together</div><div class="factor-val">{time:.0f}h / w</div></div>', unsafe_allow_html=True)
        fight = st.slider("fight", 0.0, 15.0, 1.0, key="c7")
        st.markdown(f'<div class="factor-row" style="margin:-10px 0 18px 0;"><div class="factor-label" style="color:#dc2626;">⚡ Fights / Month</div><div class="factor-val" style="border-color:#dc2626; color:#dc2626; background:#fef2f2;">{fight:.0f}</div></div>', unsafe_allow_html=True)
        happy = st.slider("happy", 0.0, 10.0, 8.8, key="c8")
        st.markdown(f'<div class="factor-row" style="margin:-10px 0 0 0;"><div class="factor-label">😊 Happy Together</div><div class="factor-val">{happy:.1f} / 10</div></div>', unsafe_allow_html=True)
...
with right:
    C = 2*math.pi*68
    O = C - (sc/100*C)
    if sc >= 70: bg, txt, label, insight = "#dcfce7", "#166534", "High Compatibility", f"BondIQ {sc}%! Elite Trust {trust:.1f} & Happy {happy:.1f}. Strong bond!"
    elif sc >= 45: bg, txt, label, insight = "#fef3c7", "#92400e", "Moderate Compatibility", f"BondIQ {sc}% — Communication {comm:.1f} can lift to 90%+"
    else: bg, txt, label, insight = "#fee2e2", "#991b1b", "Low Compatibility", f"BondIQ {sc}% — reduce fights & boost understanding."
    st.markdown(f"""
    <div class="card">
      <div style="display:flex; justify-content:space-between;"><div><div style="font-weight:800; font-size:15px;">BondIQ Result</div><div style="font-size:11px; color:#64748b;">Powered by couple_love_model.pkl • Auto</div></div><div style="width:8px; height:8px; background:#22c55e; border-radius:50%; box-shadow:0 0 12px #22c55e;"></div></div>
      ...
      <div style="background:#f8fafc; border:1px solid #e2e8f0; border-left:3px solid #0f172a; border-radius:12px; padding:12px 14px; margin-top:16px;"><div style="font-size:12px; font-weight:800; margin-bottom:4px;">💡 BondIQ Insight</div><div style="font-size:11.5px; color:#334155; line-height:1.6;">{insight}</div></div>
      <div style="margin-top:16px;"><div style="font-size:11px; font-weight:800; margin-bottom:10px;">🔍 FEATURE IMPACT</div>
        <div style="display:grid; gap:10px;">
          <div><div style="display:flex; justify-content:space-between; font-size:11px; font-weight:700;"><span>Trust {trust:.1f}</span><span>{int(trust*10)}%</span></div><div style="height:10px; background:#e2e8f0; border-radius:10px; overflow:hidden; margin-top:4px;"><div style="width:{int(trust*10)}%; height:100%; background:#0f172a;"></div></div></div>
          <div><div style="display:flex; justify-content:space-between; font-size:11px; font-weight:700;"><span>Happy {happy:.1f}</span><span>{int(happy*10)}%</span></div><div style="height:10px; background:#e2e8f0; border-radius:10px; overflow:hidden; margin-top:4px;"><div style="width:{int(happy*10)}%; height:100%; background:linear-gradient(90deg,#3b82f6,#8b5cf6);"></div></div></div>
